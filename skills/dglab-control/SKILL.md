...
Accept hardware control only through the active interaction described in [references/intent-contract.md](references/intent-contract.md). Normalize the message, validate the selected target and safety policy, then state the target and bounded command before calling the corresponding MCP tool.
...
The MCP persists a confirmed target only for its current process. Reuse it for later messages, but call `dglab_status` after APP/device removal, reconnect, relay change, or disconnect. If multiple eligible APPs or devices exist and none is selected, ask the user instead of choosing the first item.
